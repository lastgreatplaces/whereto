...
function getMonthOffsets(monthName: string) {
  const offsets: Record<string, { early: number; late: number }> = {
    Jan: { early: -1, late: 1 },
    Feb: { early: -1, late: 1 },
  ...
  return offsets[monthName] ?? { early: 0, late: 0 };
}

function getMonthPhaseTemps(monthName: string, tmax: number, tmin: number) {
  const { early, late } = getMonthOffsets(monthName);

  return {
    earlyMax: Math.round(tmax + early),
    earlyMin: Math.round(tmin + early),
    midMax: Math.round(tmax),
    midMin: Math.round(tmin),
    lateMax: Math.round(tmax + late),
    lateMin: Math.round(tmin + late),
  };
}
      ...
      {panelOpen ? (
        ...
        >
          ...
          <div style={{ fontSize: 12, color: "#555", marginBottom: 8, lineHeight: 1.4 }}>
            Select month(s), then tap the map. For travel-map shading, choose one or more U.S. states and/or Canada provinces below.
          </div>
            ...
            {travelSectionOpen && (
              <div style={{ marginTop: 10 }}>
                ...
                {allSelectedRegions.length > 0 && activeLayerMonth && (
                  <div style={{ marginTop: 6, fontSize: 11, color: "#444", fontWeight: 700, lineHeight: 1.5 }}>
                    {selectedLayerStates.length > 0 && (
                      <div>US: {selectedLayerStates.join(", ")}</div>
                    )}
                    {selectedLayerProvinces.length > 0 && (
                      <div>Canada: {selectedLayerProvinces.join(", ")}</div>
                    )}
                    <div>{MONTH_LABEL_BY_NUM[activeLayerMonth]}</div>
                  </div>
                )}
              </div>
            )}
