            ...
            <div style={{ marginBottom: 15, position: "relative" }}>
              ...
              {(placeResults.length > 0 || highwayResults.length > 0) && (
                <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: "white", border: "1px solid #ddd", borderRadius: "0 0 4px 4px", zIndex: 20, boxShadow: "0 4px 6px rgba(0,0,0,0.1)", maxHeight: "250px", overflowY: "auto" }}>
                  ...
                  {highwayResults.map((h, i) => ( <div key={i} onClick={() => { const firstCoord = h.geom_geojson.type === "MultiLineString" ? h.geom_geojson.coordinates[0][0] : h.geom_geojson.coordinates[0]; mapRef.current.setZoom(10); mapRef.current.panTo({ lat: firstCoord[1], lng: firstCoord[0] }); setSearchQuery(""); }} style={{ padding: "8px", cursor: "pointer", borderBottom: "1px solid #eee", fontSize: "11px" }}><b>🛣️ {h.name}</b></div> ))}
                </div>
              )}
            </div>
